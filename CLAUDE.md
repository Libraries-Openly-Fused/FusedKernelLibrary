...
- **Thread fusion** (vectorized loads, `core/execution_model/thread_fusion.h`): opt-in via `TransformDPP<PA, TF::ENABLED>`; requires both the first read and the last write Operation to be thread-fusion capable (`isThreadFusionCapable<Op>`). Operations never implement multi-element exec() variants: capability is declared with a `contiguous_data(params)` hook (plain pitch-linear access, e.g. PerThreadRead/Write, TensorRead/Write) or a `forwarded_access(thread, params)` hook (thread-remapping wrappers, e.g. BatchRead/BatchWrite, Circular*), and `ThreadFusionAdapter` synthesizes the wide load/store centrally. No hook (the default) disables it; any ReadBack (Crop/Resize) or fused read chain silently disables it.
