...
This is the Main branch of the repository, where the FKL API can be modified, the minimum C++ version can be increased (currently it is C++23), and in general, we can break retro-compatibility with the goal of enabling more maintainable code, and being able to fuse more types of algorithms.

If you are looking for a more stable branch, where new features can be added, but the FKL API is frozen, to ensure your user code will continue to work with newer versions, check the LTS-C++20 branch.
