...
template <> struct SimpleTransformDPPValue<ParArch::GPU_NVIDIA> {
  private:
...
template <> struct SimpleTransformDPPValueLessCallDepth<ParArch::GPU_NVIDIA> {
    static constexpr ParArch PAR_ARCH = ParArch::GPU_NVIDIA;
};
...
template <typename DPP_, typename... Operations> struct InstantiableDPP {
    using DPP = DPP_;
...
struct SimpleTransformDPPReferenceBuilder {
    template <typename... IOps> FK_HOST_FUSE auto build(const IOps &...iops) {
        return InstantiableDPP<SimpleTransformDPPReferenceFoldExpr<ParArch::GPU_NVIDIA>, IOps...>{make_tuple(iops...)};
    }
};

template <typename IDPP> __global__ void launchInstantiableDPP_Kernel(const __grid_constant__ IDPP idpp) {
    fk::apply([](auto &&...args) { return std::decay_t<IDPP>::DPP::exec(std::forward<decltype(args)>(args)...); },
              idpp.ops);
}

template <typename... IOps> __global__ void launchSimpleTransformDPPValue_Kernel(const __grid_constant__ IOps... iOps) {
    SimpleTransformDPPValue<ParArch::GPU_NVIDIA>::exec(iOps...);
}

template <size_t N, typename T> FK_HOST_DEVICE_CNST T dummyCalls(const T something) {
    if constexpr (N == 0) {
        return something;
    } else {
...
template <typename I, typename P = I, typename O = I> struct DummyOp {
  private:
  ...
  public:
    ...
    FK_HOST_DEVICE_FUSE O exec(const I input, const P params) { return static_cast<O>(input + params); }
};

template <typename ReadIOp, typename... IOps>
__global__ void launchSimpleTransformDPPValueLessCallDepth_Kernel(const __grid_constant__ ReadIOp readIOp,
...
template <> struct Executor<InstantiableSimpleTransformDPPReference> {
  private:
...
template <> struct Executor<SimpleTransformDPPValue<ParArch::GPU_NVIDIA>> {
  private:
...
template <> struct Executor<SimpleTransformDPPValueLessCallDepth<ParArch::GPU_NVIDIA>> {
  private:
...
template <> struct Executor<SimpleTransformDPPReference<ParArch::GPU_NVIDIA>> {
  private:
...
void testCompareReferenceVSValueVSInstantiableDPP() {
    ...
    executeOperations<SimpleTransformDPPReference<>>(stream, readIOp, cropIOp, resizeIOp, mulIOp, subIOp, divIOp,
                                                     colorIOp, tensorWriteIOp);

    // Second, execute the same operaitions with a transform that passes all parameters by value
    executeOperations<SimpleTransformDPPValue<>>(stream, readIOp, cropIOp, resizeIOp, mulIOp, subIOp, divIOp, colorIOp,
                                                 tensorWriteIOp);
    ...
    executeOperations<InstantiableSimpleTransformDPPReference>(stream, readIOp, cropIOp, resizeIOp, mulIOp, subIOp,
    ...
    executeOperations<SimpleTransformDPPValueLessCallDepth<>>(stream, readIOp.then(cropIOp), Cast<uchar3,
    float3>::build(), mulIOp, subIOp, divIOp, colorIOp, tensorWriteIOp); */
