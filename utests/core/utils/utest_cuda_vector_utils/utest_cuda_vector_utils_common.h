...
template <typename T, typename = void> struct can_unary_minus : std::false_type {};

template <typename T> struct can_unary_minus<T, std::void_t<decltype(-std::declval<T>())>> : std::true_type {};

template <typename T, typename = void> struct can_unary_not : std::false_type {};

template <typename T> struct can_unary_not<T, std::void_t<decltype(!std::declval<T>())>> : std::true_type {};

template <typename T, typename = void> struct can_unary_bitwise_not : std::false_type {};

template <typename T> struct can_unary_bitwise_not<T, std::void_t<decltype(~std::declval<T>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_add : std::false_type {};

template <typename T1, typename T2>
struct can_add<T1, T2, std::void_t<decltype(std::declval<T1>() + std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_subtract : std::false_type {};

template <typename T1, typename T2>
struct can_subtract<T1, T2, std::void_t<decltype(std::declval<T1>() - std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_multiply : std::false_type {};

template <typename T1, typename T2>
struct can_multiply<T1, T2, std::void_t<decltype(std::declval<T1>() * std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_divide : std::false_type {};

template <typename T1, typename T2>
struct can_divide<T1, T2, std::void_t<decltype(std::declval<T1>() / std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_equal : std::false_type {};

template <typename T1, typename T2>
struct can_equal<T1, T2, std::void_t<decltype(std::declval<T1>() == std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_not_equal : std::false_type {};

template <typename T1, typename T2>
struct can_not_equal<T1, T2, std::void_t<decltype(std::declval<T1>() != std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_less : std::false_type {};

template <typename T1, typename T2>
struct can_less<T1, T2, std::void_t<decltype(std::declval<T1>() < std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_less_equal : std::false_type {};

template <typename T1, typename T2>
struct can_less_equal<T1, T2, std::void_t<decltype(std::declval<T1>() <= std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_greater : std::false_type {};

template <typename T1, typename T2>
struct can_greater<T1, T2, std::void_t<decltype(std::declval<T1>() > std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_greater_equal : std::false_type {};

template <typename T1, typename T2>
struct can_greater_equal<T1, T2, std::void_t<decltype(std::declval<T1>() >= std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_logical_and : std::false_type {};

template <typename T1, typename T2>
struct can_logical_and<T1, T2, std::void_t<decltype(std::declval<T1>() && std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_logical_or : std::false_type {};

template <typename T1, typename T2>
struct can_logical_or<T1, T2, std::void_t<decltype(std::declval<T1>() || std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_bitwise_and : std::false_type {};

template <typename T1, typename T2>
struct can_bitwise_and<T1, T2, std::void_t<decltype(std::declval<T1>() & std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_bitwise_or : std::false_type {};

template <typename T1, typename T2>
struct can_bitwise_or<T1, T2, std::void_t<decltype(std::declval<T1>() | std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_bitwise_xor : std::false_type {};

template <typename T1, typename T2>
struct can_bitwise_xor<T1, T2, std::void_t<decltype(std::declval<T1>() ^ std::declval<T2>())>> : std::true_type {};

// Compound assignment operators
template <typename T1, typename T2, typename = void> struct can_add_assign : std::false_type {};

template <typename T1, typename T2>
struct can_add_assign<T1, T2, std::void_t<decltype(std::declval<T1 &>() += std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_sub_assign : std::false_type {};

template <typename T1, typename T2>
struct can_sub_assign<T1, T2, std::void_t<decltype(std::declval<T1 &>() -= std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_mul_assign : std::false_type {};

template <typename T1, typename T2>
struct can_mul_assign<T1, T2, std::void_t<decltype(std::declval<T1 &>() *= std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_div_assign : std::false_type {};

template <typename T1, typename T2>
struct can_div_assign<T1, T2, std::void_t<decltype(std::declval<T1 &>() /= std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_and_assign : std::false_type {};

template <typename T1, typename T2>
struct can_and_assign<T1, T2, std::void_t<decltype(std::declval<T1 &>() &= std::declval<T2>())>> : std::true_type {};

template <typename T1, typename T2, typename = void> struct can_or_assign : std::false_type {};
...
template <typename I1, typename I2> void detectBinaryUnexpectedCompilationErrors() {
    if constexpr (cn<I1> == cn<I2>) {
        ...
        if constexpr (can_less_equal<I1, I2>::value != can_less_equal<VBase<I1>, VBase<I2>>::value) {
            unexpected_failed_compilations.push_back("binaryLessEqual_" + typeToString<I1>() + "_" +
                                                     typeToString<I2>());
        }
        ...
        if constexpr (can_greater_equal<I1, I2>::value != can_greater_equal<VBase<I1>, VBase<I2>>::value) {
            unexpected_failed_compilations.push_back("binaryGreaterEqual_" + typeToString<I1>() + "_" +
                                                     typeToString<I2>());
        }
...
template <typename I1, typename I2> bool testBinaryOperators() {
    constexpr std::array<std::string_view, 15> binaryOperatorTestNames{
        ...
        "binaryLogicalAnd", "binaryLogicalOr", "binaryBitwiseAnd", "binaryBitwiseOr", "binaryBitwiseXor"};
    constexpr std::array<bool, 15> results = {
        binaryadd<I1, I2>(),           binarysubtract<I1, I2>(),    binarymultiply<I1, I2>(),
        binarydivide<I1, I2>(),        binaryequal<I1, I2>(),       binarynot_equal<I1, I2>(),
        binaryless<I1, I2>(),          binaryless_equal<I1, I2>(),  binarygreater<I1, I2>(),
        binarygreater_equal<I1, I2>(), binarylogical_and<I1, I2>(), binarylogical_or<I1, I2>(),
        binarybitwise_and<I1, I2>(),   binarybitwise_or<I1, I2>(),  binarybitwise_xor<I1, I2>()};
    bool correct{true};
    for (int i = 0; i < 15; ++i) {
        if (!results[i]) {
            std::cout << "Failed " << binaryOperatorTestNames[i] << " test for types: " << typeToString<I1>() << " and "
                      << typeToString<I2>() << std::endl;
            correct = false;
        }
    }
    return correct;
...
template <typename I1, typename I2> bool testCompoundOperators() {
    ...
    for (int i = 0; i < 6; ++i) {
        if (!results[i]) {
            std::cout << "Failed " << compoundOperatorTestNames[i] << " test for types: " << typeToString<I1>()
                      << " and " << typeToString<I2>() << std::endl;
            correct = false;
        }
    }
    return correct;
