...
/* FUSED READ IOps + FUSED WRITE IOp in the same run:
   Q prologue read.then(Mul(2)), V prologue read.then(Mul(3)).then(Add(1)),
   output written through Mul(2).then(Add(0.5)).then(makeAttentionWrite(...)).
   Since sum p_j = 1: attn(2Q, K, 3V+1) = 3*attn(2Q, K, V) + 1, and the fused
   write chain gives out = 2*(3*ref + 1) + 0.5 = 6*ref + 2.5.
   Called with big shapes too, to span many CUDA blocks/threads. */
template <int HEAD_DIM>
static void testFusedReadWrite(const char* name, const int bh, const int seqQ,
    ...
    std::vector<float> hq(nQ), hk(nK), hv(nK);
    std::vector<double> dq(nQ), dk(nK), dv(nK), ref;
    for (size_t i = 0; i < nQ; ++i) { hq[i] = dist(rng); dq[i] = 2.0 * hq[i]; }
    for (size_t i = 0; i < nK; ++i) { hk[i] = dist(rng); dk[i] = hk[i]; }
    for (size_t i = 0; i < nK; ++i) { hv[i] = dist(rng); dv[i] = hv[i]; }
    ...
    executeFlashAttention<HEAD_DIM>(qIOp, kIOp, vIOp, oIOp, bh, seqQ, seqK,
                                    causal, stream);
    stream.sync();
...
int launch() {
    ...
    testFusedEpilogue();
    testFusedPrologue();
    // fused Read IOps + fused Write IOp, small and BIG shapes (the big ones
    // span many CUDA blocks: grid.x = seqQ/WARPS_PER_BLOCK, grid.y = bh).
    testFusedReadWrite<32>("FA fused R/W IOps d32 b2 s24/s48", 2, 24, 48, false, 5e-5, 7);
