...
FusedKernelLibrary (FKL) is a header-only C++20 library for automatic GPU kernel fusion (Vertical, Horizontal, Backwards Vertical, and Divergent Horizontal Fusion), with CPU CUDA and HIP  backends. Only nvcc is supported as the CUDA compiler. `main` is the development branch (v0.2.0, API may break); `LTS-C++17` is the frozen-API branch.
...
Requires CMake >= 3.28, a C++20 host compiler, and CUDA (nvcc) or HIP (amdclang). Project policy and CI treat CUDA or HIP as required, but CMake degrades gracefully: without nvcc or amdclang it configures CPU-only and generates only `*_cpp` targets.
